...
Mat get_hogdescriptor_visual_image(Mat& origImg,
  ...
  int main(int argc, char **argv)
  {
    ...
    ROS_INFO("Found %d positive samples", (int) positiveTrainingImages.size());
    ROS_INFO("Found %d negative samples", (int) negativeTrainingImages.size());

    int overallSamples = positiveTrainingImages.size() + negativeTrainingImages.size();

    // Make sure there are actually samples to train
    if (overallSamples == 0) {
      ROS_WARN("No training sample files found, nothing to do!\n");
      return EXIT_SUCCESS;
    }
    ...
    Mat labelsMat(overallSamples, 1, CV_32FC1, Scalar(-1.0));
    labelsMat.rowRange( 0, positiveTrainingImages.size() ) = Scalar( 1.0 );

    Mat feat_matrix; //TODO compute this

    ROS_INFO("Reading files, generating HOG features and save them to file '%s':\n", featuresFile.c_str());
    float percent;
    ...
    for (unsigned long currentFile = 0; currentFile < overallSamples; ++currentFile) {
      ...
      ROS_DEBUG_THROTTLE_NAMED(60, "PERCENT", "Processing: %5lu (%3.0f%%):\tFile '%s'", (currentFile+1), percent, currentImageFile.c_str());
      ...
      if (!featureVector.empty()) {
        ...
        */
        if(feat_matrix.cols ==0){
          feat_matrix = Mat(featureVector);
        }else{
          cv::hconcat(feat_matrix, Mat(featureVector), feat_matrix);
        }
      }
    }

    transpose(feat_matrix, feat_matrix);

    ROS_INFO("Resulting feature matrix for training:");
    ROS_INFO("rows: %d", feat_matrix.rows);
    ROS_INFO("cols: %d", feat_matrix.cols);

    ROS_INFO("Resulting label matrix for training:");
    ROS_INFO("rows: %d", labelsMat.rows);
    ROS_INFO("cols: %d", labelsMat.cols);
    ...
    // Train the SVM
    LinearSVM svm;
    svm.train(feat_matrix, labelsMat, Mat(), Mat(), params);

    svm.save(svmModelFile.c_str());
    ...
    ROS_INFO("Hog Detector settings saved under: %s", cvHOGFile.c_str());
    double hitThreshold = 0; //Just ommit this
    ROS_INFO("Testing training phase using training set");
    ...
    vector<string> imgs;
    imgs.push_back("imgs/hazmat.jpg");
    ...
    Size block_stride=Size(4,4);
    Size cell_size=Size(4, 4);
    ...
    for(int i=0; i< imgs.size(); ++i)
    {
      ...
      //resizing
      resize(img, img, win_size ); //Size(64,48) ); //Size(32*2,16*2)); //Size(80,72) );
      //gray
      cvtColor(img, img_gray, CV_RGB2GRAY);

      //extract feature
      HOGDescriptor d( win_size, block_size, block_stride, cell_size, 9);
      ...
      Mat hog_vis = get_hogdescriptor_visual_image(img, descriptorsValues, win_size, cell_size, 5, 1);
    ...
    FileStorage hogXml("config/hog.xml", FileStorage::WRITE); //FileStorage::READ
    //2d vector to Mat
